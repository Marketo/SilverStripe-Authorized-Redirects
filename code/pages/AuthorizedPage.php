    ...
    public function getCMSFields()
    {
        ...
        $fields->addFieldToTab('Root.Main',
            TextField::create('RedirectURL')
                ->setDescription('Instead of including content on the page, redirect to an external URL upon authentication.'), 'Content');
...
class AuthorizedPage_Controller extends Page_Controller
{
    ...
    public function index()
    {
        if (!$this->Authorization($this->request->getVar('Email'), $this->request->getVar('AccessCode'))) {
            ...
            if ($Errors & Authorization::EMAIL_EMPTY) {
            ...
            } elseif ($this->request->getVar('EmailSent') !== null) {
                ...
                } elseif ($Errors & Authorization::EMAIL_NOT_ALLOWED) {
                    $this->data()->addMessage('This email address is not allowed access to this page.', 'error');
                } elseif ($Errors & Authorization::PAGE_HAS_NO_EMAIL) {
                    $this->data()->addMessage('Welcome back!', 'error');
                } elseif ($Errors & Authorization::DEVICE_WRONG) {
                    //$this->data()->addMessage($ShowErrorCode, 'error');
                ...
                } elseif (!($Errors & Authorization::ACCESS_CODE_MISSING)) {
                    //$this->data()->addMessage($ShowErrorCode, 'error');
    ...
    {
        if (!is_null($this->AuthorizationErrors)) {
